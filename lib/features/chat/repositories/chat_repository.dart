...
class ChatRepository{
  ...
  Stream<List<Group>> getChatGroups(){
     print('Obteniendo grupos de Firestore...');
    return firestore
      .collection('groups')
      .snapshots()
      .map((event) {
  ...
  void _saveDataToContactsSubcollection(
    ...
    if(isGroupChat){
      ...
      await firestore.collection('users').doc(recieverUserId).collection('chats').doc(auth.currentUser!.uid).set(recieverChatContact.toMap(),
      );
  ...
  void _saveMessageToMessageSubcollection({
    ...
    if(isGroupChat){
      await firestore.collection('groups').doc(recieverUserId).collection('chats').doc(messageId).set(message.toMap());
    }else{
      await firestore
    .collection('users')
    .doc(auth.currentUser!.uid)
  ...
  void sendFileMessage({
    ...
    try{
      ...
      String imageUrl= await ref.read(commonFirebaseStorageRepository).storeFileToFirebase('chat/${messageEnum.type}/${senderUserData.uid}/$recieverUserId/$messageId',file);
      ...
      _saveDataToContactsSubcollection(senderUserData, recieverUserData, contactMsg, timeSent, recieverUserId, isGroupChat);
